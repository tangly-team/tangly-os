...
The property _entityFid_ is the foreign key declared in class _ReferenceType_ to implement the relation. The SQL table of _ReferenceType_ has a
column _BIGINT entityFid_.
The boolean parameter states that the instance at the end of the relation is owned.
...
.Add JSON Property
The _Value_ class is the type of the values to be converted to JSON for storage in the database and converted to Java objects when retrieved from
the database. The transformation process uses the Jackson library.

The _jsonValues_ relation shall be a collection in the Java class. The SQL column declaration has the form _jsonValues CLOB_.
[source,java]
----
