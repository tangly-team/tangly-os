...
The product launch is a tremendeous success.
...
It implies https://en.wikipedia.org/wiki/Specification_by_example[specifications by example] - you need to train your business analysts and product owners,
...
For instance, on a previous outcome, we had planned a coding dojo ritual once every two weeks, during which we shared our practices with some perspective over the outcome.
It was an occasion to experiment new technologies, assess their match for the outcome needs, share new coding techniques, and update our standards together.
...
* Craftsmanship is not enough to guarantee the success of a outcome but the lack of it can be the main cause of its failure,
