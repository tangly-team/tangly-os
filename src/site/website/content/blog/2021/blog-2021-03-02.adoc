...
footnote:[I have reported the bug how they used path instances. The developers were very supportive. The version 4.01 supports in-memory file systems such as
JIMFS.].
...
footnote:[I have reported the bug in the microstream forum and informed them that their clain they support these JDK is plain wrong. The project sadly does
not currently have a bug reporting system. We hope that the version promised for April 2021 will finally solved these show stoppers.].
