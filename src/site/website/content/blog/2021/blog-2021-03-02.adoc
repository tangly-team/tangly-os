...
tags: ["java", "design"]
params:
  featured: "blog/2021/pics/2021-03-02-head.jpg"
---
...
The library provides a cool and compact approach to persisting complex data models.
...
I use the library to prototype bounded domain implementations requiring persistent data.
...
Saving a collection::
If you have access to the collection, call the store operation on the collection.
If the collection is hidden inside an object, you are forced to use an _eager_ store.
...
footnote:[I have reported the bug how they used path instances. The developers were very supportive. Version 4.01 supports in-memory file systems such as https://github.com/google/jimfs[Jimfs].].
...
The Windows file system implements some Windows’ quirks, such as not permitting the creation of a file in the root directory.
...
A field cannot be manipulated anymore in record since Java JDK 1.5.1.
...
For example, IntelliJ IDEA stops providing support for obsolete JDKs as soon as the new official JDK release is available.
It hinders you from using the associated preview features
footnote:[I have reported the bug in the microstream forum and informed them that their claim they support these JDK is plain wrong. The project sadly does not currently have a
bug reporting system. I hope the version promised for April 2021 will finally solve these showstoppers.]
footnote:[Version 5.0 was released in July 2021 and finally provides support for records. You need to increase access to internal components with the option
...
* Wait for the release scheduled for April 2021 and pray they will support the record feature.
Records are an official feature in JDK 16 released in March 2021 and no more than a preview feature.
See our link:../../2021/modern-java-development/[Modern Java Development]
