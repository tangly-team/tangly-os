...
You just need to take care of one build configuration and no more need to delve into the mysteries of the IntelliJ build process.

The only glitch is that IntelliJ has its own concept of modules and is not synchronized with the Gradle modules.
...
I use the following configuration to generate an aggregated coverage report over all Gradle modules.
...
External tools no longer support the binary report.
<4> -
...
The new approach to creating aggregated reports is described in link:../../2022/multi-module-test-reporting-with-gradle-7.x/[Multi-module Test Reporting with Gradle 7].
====
