...
image::2023-03-02-head.png[width=420,height=360,role=left]
...
Seminal books<<accelerate>> <<devops-handbook>> <<phoenix-project>> <<site-realibility-engineering>> and good practices were published around the same time.
...
The metrics for throughput are _deployment frequency, lead time for changes_.
The metrics for stability are _mean time to recover, change failure rate_.
...
Frequent and economical deployments require the full automation of integration, delivery and deployment steps.
...
It is the reason why DORA metrics measure _mean time to recover_  and _change failure rate_.
...
image::2023-03-02-ci-vs-cd.png[width=90%,height=360,role=text-center]
...
Incremental migration of database schema and productive data is a well-understood software engineering problem <<refactoring-database>> for decades.
...
If you test configuration changes in staging branches, and you want them to be applied in the production, you have to either:
...
_This approach is compatible to clean code and DevOps_.
* Pass them manually from your staging to your production database, by copying and pasting them.
_This approach is probably a motive to fire the developer because auditability and reproducibility are not enforced_.
...
The above process is documented for odoo version 15 under https://www.odoo.com/documentation/15.0/administration/odoo_sh/getting_started/branches.html[branches].
...
The statement violates all rules of clean code, DevOps and professional software development.
...
Staging and production branches are effectively deployment triggers in odoo.sh world.
...
footnote:[If you write, for example, an SQL database schema migration script, you use clauses such *DROP IF EXISTS*.].
...
We politely ask you to read books and documentation. Avoid receiving a https://en.wikipedia.org/wiki/RTFM[RTFM] injunction.
====

You need some expertise in Docker, Kubernetes and in your development platform <<devops-sme>>.
We recommend to initally use GitHub.
It is the most used platform and most developers are versed how to use the provided features.
...
Often, advanced concepts such as monitoring, DevSecOps, license management are supported.
...
- [[[site-realibility-engineering, 5]]] https://www.amazon.com/dp/B01DCPXKZ6[Site Reliability Engineering: How Google Runs Production Systems].
Niall Richard Murphy, Betsy Meyer, Chris Jones.
O'Reilly. 2016
- [[[refactoring-database, 6]]] https://www.amazon.com/dp/B001QAP36E[Refactoring Databases: Evolutionary Database Design].
Scott W. Ambler.
Addison-Wesley. 2006
- [[[devops-sme, 7]]] http://blog.tangly.net/blog/2021/devops-for-small-applications/[DevOps for Small Applications]
Marcel Baumann. 2021
- [[[code-as-crime-scene, 8]]] link:../../2019/code-scene-as-crime-scene/[Code Scene as a Crime Scene]
Marcel Baumann, 2019
- [[[detecting-agile-bullshit, 9]]] link:../../2019/detecting-agile-bullshit/[Detecting Agile Bullshit]
Marcel Baumann, 2019
