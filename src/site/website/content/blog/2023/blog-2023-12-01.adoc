...
{ref-gradle} build tool is a fast, dependable, and adaptable open-source build automation tool with an elegant and extensible declarative build language.

Gradle is a popular build system for the JVM and is the default system for Android and Kotlin Multi-Platform projects.

It has a rich community plugin ecosystem.

Gradle is fast, scalable, and can build projects of any size and complexity.

Long ago I migrated all my projects from Maven to Gradle.
...
footnote:[I had to wait until {ref-vaadin} provided an official supported Gradle plugin.].
--
...
The {ref-gradle} team has learned a lot and has improved the tooling.

As a drawback, I had to regularly refactor my scripts to keep them up to date.
====
...
Just use the latest version and run your unit and integration tests.
