...
A battery replacement on a MacBook notebook easily takes one week in Switzerland.
...
footnote:[I love the bibtex to keep track of all my literature references.].
...
The official Swiss railway company does not provide Internet access on the trains.
...
<2> Documentation tools are used to produce all technical documents.
...
Remember to create the link to the java installation.
...
All developers in our company have been using Apple notebooks since the company's inception.
...
The Homebrew package manager streamlines installation and update of necessary software packages.

[TIP]
====
An incredible useful feature is a time machine.
...
The built-in loudspeakers are an audible improvement against the sound of the previous machine.
