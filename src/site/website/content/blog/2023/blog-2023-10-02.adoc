...
Data migration::
How is persisted data migrated when a new application version is deployed?s
...
Batch processing::
Should we process batch of requests?
Business rules::
...
Communication between bounded domains is asynchronous and message based.
...
[IMPORTANT]
====
Monitoring is essential in a sucessful product.
Do not skip it.
