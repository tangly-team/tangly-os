...
[TIP]
====
C++ has way better support for unit and integration testing.
The libraries are easier to use and better maintained.

Modern approaches like TDD and BDD can be used.
The quality of the delivered solution is often markantly improved with these practices.
====
...
[CAUTION]
====
The C language and C++ language have added support for multi-threaded and synchronization primitives since version 11.

This standard API could become the preferred aproach to develop multi-threaded and realtime application.
