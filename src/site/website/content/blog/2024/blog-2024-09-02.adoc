...
The software is typically specialized for a particular hardware with specific purpose that must meet time, size, energy, and memory constraints.
...
The embedded software must be capable of detecting faults and make decisions according to how critical a fault is.
...
RTOS can not be run in the unprivileged mode because they need access to the processor registers and interrupts.
...
The supervisor needs to provide interrupt handling, timers, task switch and potentially access to privileded instructions or locations.It must transfer data to and receive from the scheduler.
