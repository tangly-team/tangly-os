...
A web server library such as https:/eclipse.dev/jetty/[Jetty] supports unlimited parallel user sessions without writing one line of code.
...
[TIP]
====
Try to archive data in a format that is easy to read and understand.
I strongly recommend a TSV and JSON format.
Pleaes provid JSON schema for the JSON data.
