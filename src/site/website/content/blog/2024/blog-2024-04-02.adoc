...
:ref-jetty: https://www.eclipse.org/jetty/[Jetty]
...
I love IntelliJ IDEA, but it is slow to support new Java versions.
...
Gradle is my preferred build tool.
...
The standard scripts do not support installing multiple JDK versions in the image building your application
footnote:[If you have a code snippet to install multiple JDKs, run Gradle with an older version and let Gradle toolchain find the current JDK to build, let me know.
I will add your solution with name in this blog.]._
...
It is a small and lightweight framework and uses the {ref-jetty} server.
...
_Please remember never to use internal classes of a library in your code.- _It will bite you._

_Update your JDK every three months to install the latest security patches._

_Update continously your dependencies to the latest stable version._
