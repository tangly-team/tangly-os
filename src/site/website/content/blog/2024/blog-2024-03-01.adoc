...
opens _package list_::: If we need to allow reflection of private types, but we don’t want all of our code exposed, we can use the opens directive to expose specific packages.
...
<3> Optional locator responsible to find all implementations of a specific service interface
<4> Consumer calling an implementation of the service.
If you do not implement an explicit locator, the consumer is responsible to find a suitable implementation.
