...
_A major gaal is to avoid any taintinng of the core domain model from other layers._
_Try avoiding extending domain entities with implementation specific root classes or to a lesser degree annotations._
Infrastructure::
...
The persistence functionality is less than fifty lines of codes.
No changes in the domain model was necessary.
We did not had to add a root persistent class or use annotations._
