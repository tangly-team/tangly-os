...
params:
  featured: "blog/2020/pics/2020-11-01-head.jpg"
---
...
We view reference codes more as strongly typed and efficient tags.
...
Additional information is available under the link:../../../docs/bus/businessmodels/ Below the source code in modern Java is
...
Once your enumeration types implement the code interface, you are free to migrate selected types to external persistent and extensible solutions.
...
The JSON files containing the code values can be stored as resource files and delivered either with the application or in a separate jar file.
...
You have the freedom to model a set of values as enumeration, reference codes, or tags.
Each approach has specific advantages and tradeoffs.
The costs of moving from one approach to another are quite small.
Choose one solution, implement it, and track its adequacy and user acceptance.
