...
But Scrum also states we have a vision, features, a minimum viable article and a potentially shippable article.
...
Bend it with Kanban - for maintenance activities -, Agile approach is about a minimum viable article release as soon as possible.
...
image:2017-01-01-Meeting.jpg[720, 480, role=left]
Writing user stories is easy and anyway nobody has time for - The article owner has better to do.
...
Scrum states the article backlog is the most important document in a Scrum product.

[TIP]
====
_ROI: New features with the most value_ To create a new successful article is a full time job.

You cannot define a vision and key features during a coffee break.
The article owner must create a vision, an initial release plan, identify the key features and define a minimal shippable article - see above "You shall not perform up-front design" -, Either the article owner has a team of requirement engineers to elicit the use cases, Or the role of requirement engineering is part of the team, The team provide technical feedback and input about potential technologies for all stories, discuss the non-functional requirements and refine the acceptance criteria, As a simple check, the team guaranty together with the article owner that each story is INVEST - Independent, Negotiable, Valuable (ROI), Estimable, Size appropriately, Testable -.
====
...
You can use it to manage different things, including complex article development.
