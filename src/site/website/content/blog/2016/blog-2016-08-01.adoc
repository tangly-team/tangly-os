...
.. Envision,
.. Speculate (During the Speculate phase, the article stories are delivered.
...
.. Close.
.. Scope, Costs, Time
.. http://jimhighsmith.com/2010/11/14/beyond-scope-schedule-and-cost-the-agile-triangle/[The agile triangle]: Value (releasable article),
 Quality (reliable, adaptable article),  Constraints (cost, schedule, scope)
...
** Threshold or basic attributes are must have attributes otherwise the article is incomplete.
 Threshold features are those that must be present in the article  for it to be successful. They are often referred to as must-have features.
...
* Scrum uses the sashimi technique to require that every slice of functionality created by the developers be complete.
 All of the requirements gathering and analysis, design work, coding, testing, and documentation that constitute a complete article are required to be completed
...
* Iterative development means that we build a partial version of a article and then expand that version through successive short time periods of development
 followed by reviews and adaptations. Feature-based delivery means that the engineering team builds features of the final article or, particularly with
 industrial products. At least a close representation of the final article (such as a simulation model). Iterations are constrained to produce a result within
...
* Forecasting the financial value of a theme is the responsibility of the article owner, but it is a responsibility shared with all other team
