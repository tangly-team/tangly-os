 ...
 * The following rules are applied. <uL> <li>Only one transition is eligible upon processing of an
 * event otherwise the state machine behavior is undefined because the machine is not deterministic
 * as expected in hierarchical Harel state machines.</li> <li>No exception should be thrown during
 * the processing of a condition or an action.</li> </uL> <p> For each violation an illegal state
...
public class DynamicChecker<O, S extends Enum<S>, E extends Enum<E>> implements StateMachineEventHandler<O, S, E> {
     ...
     *
     * @param fsm finite state machine to check dynamically
     */
