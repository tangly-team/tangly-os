...
/**
 * <p>The package defines the interfaces to access hierarchical state machines. The interfaces are designed to provide an immutable representation
 * of finite state machine definition.</p>
 ...
 * The active state configuration is the tree of active states; in particular, for every concurrent composite state each of its orthogonal regions
 * is active. The event queue holds the events that have not yet been handled by the machine.</p>
 ...
 * <p>The least common ancestor (LCA) of the enabled transition is determined, i.e. the lowest composite state that contains all the transition’s
 * source and target states. The transition’s main source state, that is the direct substate of the LCA containing the source states, is
 * deactivated, the transition’s actions are executed, and its target states are activated.</p>
 * <h2>Decisions</h2>
