...
A transition defines a potential change from one state to another one. A transition has a mandatory event trigerring the firing of transition and a
...
* when a state is exited throught the firing of a transition
